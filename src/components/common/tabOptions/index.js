  ...
  return (
    <div className='tab-options'>
      <div className='max-width options-wrapper'>
        {tab.map((tabs)=>{
          return (
            <div onClick={()=>setActiveTab(tabs.name)}
            className={`tab-item absolute-center cur-po ${activeTab===tabs.name && 'active-tab'}`}>
              <div className='tab-image-container absolute-center'
              style={{backgroundColor: `${activeTab===tabs.name?tabs.backdrop:""}`}}>
                <img className='tab-image' alt={tabs.name}
                src={activeTab===tabs.name? tabs.active_img: tabs.inactive_img}/>
              </div>
